...
def is_excluded(host):
    """Check if a host is excluded from blacklist based on the whitelist and filter non ascii"""
    return any(host.endswith(allowed) for allowed in whitelist) or any(ord(ch) >= 128 for ch in host)

def read(host_url=AD_HOSTS_URL) -> set:
    ...
    print('# dnsmasq host list generated by {}'.format(sys.argv[0]))
    print('# source {}'.format(host_url))
    for blockhost in sorted(host_set):
        print('address=/{}/'.format(blockhost))
    return host_set
